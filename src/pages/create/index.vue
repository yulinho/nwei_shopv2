<template>
  <div class="container_class">
    ...
    <div class="container_coupons_class">
      ...
      <div class="container_coupons_content_class">
        ...
        <div class="container_coupons_content_content_class">
          <van-cell-group>
            <van-field label="标题" v-model="title" @change="onChangetitle" placeholder="输入100字以内的活码标题" />
          </van-cell-group>
          <div class="container_coupons_content_content_text1_class as_smallgray" v-if="true">
            显示扫描后的标题(允许留空)
          </div>
          <van-cell-group v-if="true">
            <van-field label="阈值" v-model="threshold" @change="onChangethreshold" placeholder="1以上数字" />
          </van-cell-group>
          <div class="container_coupons_content_content_text1_class as_smallgray" v-if="true">
            达到阈值将切换到下一个二维码
          </div>
          ...
          <div class="container_coupons_content_content_charge_class as_smallgray">
            <!-- 需支付¥{{charge}}服务费 -->
            <!-- 您说的语音会识别在这里 -->
          </div>
          <div v-if="!hmbbhuoma._id">
            <button class="container_coupons_content_content_submit_class china_red" @click="onClickStep1"><span class="container_coupons_content_header_say_button_iconfont_class ping">&#xe6f7;</span> 第一步,保存</button>
          </div>
          <div v-if="hmbbhuoma._id">
            <button class="container_coupons_content_content_submit_class china_red" @click="onClickStep1">保存</button>
          </div>
        </div>
        <div v-if="hmbbhuoma._id" class="container_coupons_content_addqrcode_class">
          <button class="container_coupons_content_content_submit_class china_red" @click="onClickStep2"><span class="container_coupons_content_header_say_button_iconfont_class ping">&#xe74a;</span> 第二步,添加二维码图片</button>
        </div>
        <div class="container_coupons_content_brand_class">
          ...
          <div class="container_coupons_content_scroller_item_image_class">
            ...
            <img v-if="hmbbhuoma._id" class="container_coupons_content_scroller_item_image_image_class" :src="hmbbhuoma.qrcode" @click="previewImage(hmbbhuoma.qrcode)">
            <div class="container_coupons_content_scroller_item_image_text_class" v-if="!hmbbhuoma._id">
              <text class="container_coupons_content_scroller_item_image_text_text_class">&#xe605;</text>
            </div>
            <!-- <div id="siri-container" class="container_coupons_content_scroller_item_image_wave_class"> -->
          </div>
          <div v-if="hmbbhuoma._id" class="container_coupons_content_brand_download_class" @click="onClickSave"><span>&#xe600;</span>下载活码</div>
        </div>
    ...
    <div>
      <Item :args="item" @clickDesc="item.onClickDesc" v-for="(item,index) in items" :key="index" />
    </div>
...
export default {
  ...
  methods: {
    ...
    async fetch_hmbbqrcodes() {
      this.items = []
      let nsp = 'hmbbqrcodes_get'
      let res = await t.v2dispatch({
        type: 'v2hmbbopen',
        payload: {
          hmbbhuoma__id: this.hmbbhuoma._id,
          nsp,
        },
      })
      // console.log(res.data);
      let hmbbqrcodes = res.data
      for (var i = 0; i < hmbbqrcodes.length; i++) {
        let hmbbqrcode = hmbbqrcodes[i]
        this.items.push({
          pict_url: hmbbqrcode.url,
          title: `序号: ${i+1}`,
          tag: `已扫 ${hmbbqrcode.pv} 次`,
          subdesc: t.formatTime(hmbbqrcode.created_at),
          desc: '点击删除',
          onClickDesc: async () => {
            // console.log(hmbbqrcode);
            let nsp_hmbbqrcode = 'hmbbqrcode_update'
            hmbbqrcode.trash = 1
            let res_hmbbqrcode = await t.v2dispatch({
              type: 'v2hmbbopen',
              payload: {
                hmbbqrcode: hmbbqrcode,
                nsp: nsp_hmbbqrcode,
              },
            })
            // console.log(res_hmbbqrcode);
            ctx.items = []
            await ctx.fetch_hmbbqrcodes()
          }
        })
      }
    ...
    async onClickStep2() {
      ...
      // var that = this;
      // 选择图片
      let options = await t.getItem(`qnoptions`)
      ...
      if (url) {
        ...
        let data = res.data
        let hmbbqrcodes = res.hmbbqrcodes
        console.log(data);
    ...
    async onClickStep1() {
      ...
      //   hmbbuser__id,
      // }
      this.hmbbhuoma.title = this.title
      this.hmbbhuoma.threshold = this.threshold
      this.hmbbhuoma.hmbbuser__id = hmbbuser__id
      let nsp = 'hmbbhuoma_update'
      let res = await t.v2dispatch({
        type: 'v2hmbbopen',
        payload: {
          // hmbbhuoma: JSON.stringify(hmbbhuoma),
          hmbbhuoma: this.hmbbhuoma,
          nsp,
        },
      })
      let data = res.data
